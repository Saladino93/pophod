...
class WebSky:
    ...
    def import_astropy(self):
        ...
        self.u = u
        self.astropy_cosmo = FlatLambdaCDM(H0=self.websky_cosmo['h']*100, Om0=self.websky_cosmo['Omega_M'])#m_nu = 0.0
    ...
    def load_halo_catalogue(self, mmin=0., mmax=np.inf, zmin=0., zmax=np.inf, rmin=0., rmax=np.inf, practical=True, Nmax=None):
        ...
        rho_mean = 2.775e11 * self.websky_cosmo['Omega_M'] * self.websky_cosmo['h']**2 #Msun/Mpc^3
        halodata[:,6] = 4.0/3*np.pi * halodata[:,6]**3 * rho_mean #Msun      
...
def catalogue_to_pix(halodata, nside = 512):
...
def pix_to_map(pix, nside = 512, weight = 1,):
    """project halos into healpix map of size nside
    Parameters
    ----------
    pix: np.ndarray
...
def catalogue_to_map(halodata, nside = 512, weight = 1, return_pix: bool = False):
    """project halos into healpix map of size nside
    Parameters
    ----------
    halodata : array
    ...
    result = map if not return_pix else (map, pix)

    return result


def get_pix_shape_from_halo_data_and_nside(halodata, nside):
    return hp.vec2pix(nside, halodata[:,0], halodata[:,1], halodata[:,2]), hp.nside2npix(nside)


def jax_catalogue_to_map(pix, shape, weight = 1, return_pix: bool = False):
    """project halos into healpix map of size nside
    Parameters
    ----------
    halodata : array
